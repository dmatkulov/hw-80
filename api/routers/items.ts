...
itemsRouter.get('/:id', async (req, res, next) => {
  try {
    const [results] = await mysqlDb.getConnection().query(
      'SELECT id, category_id, place_id, name, description, image FROM items WHERE id = ? ',
      [req.params.id]
    ) as RowDataPacket[];
...
itemsRouter.put('/:id', imageUpload.single('image'), async (req, res, next) => {
  ...
  try {
    ...
    await mysqlDb.getConnection().query(
      'UPDATE items SET `category_id` = ?, `place_id` = ?, `name` = ?, `description` = ?, `image` = ?' +
      'WHERE id = ? ',
      [item.categoryId, item.placeId, item.name, item.description, item.image, itemId]
    );
